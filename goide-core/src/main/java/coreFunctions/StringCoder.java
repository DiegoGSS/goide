...
public class StringCoder {
    ...
    public String decodeString(String inputString){
        ...
        String outputString  = "";
        for (int i = 0, n = inputString.length(); i < n; i++) {
            outputString+= this.characterCoder.cesarDecodeCharacter(inputString.charAt(i));
        }
        return outputString;
    }
    ...
    public String decodeString(String inputText, String key){
        String outputString  = "";
        BeautifulString inputBeautifulString = new BeautifulString(inputText);
        BeautifulString keyBeautifulString = new BeautifulString(key);
        while(!inputBeautifulString.hasLaps()){
            if(CharacterUtils.isNotLetterCharacter(inputBeautifulString.iterate())){
                ...
                outputString+= this.characterCoder.decodeCharacter(inputBeautifulString.getCurrentChar(), keyBeautifulString.iterate());
            }
        }
        return outputString;
    }}
